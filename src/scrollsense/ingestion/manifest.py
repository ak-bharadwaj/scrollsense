...
class ReelAssetManifestItem(BaseModel):
    ...
    asset_path: str = Field(..., description="Path to the stored media asset file")
    source_url: str | None = Field(default=None, description="Original source URL or repository link")
    source_platform: str = Field(..., description="Source platform (local_filesystem, instagram, etc.)")
    license: str = Field(..., description="Explicit content license (e.g., CC-BY-4.0, MIT, Educational Fair Use)")
    ...
    transcript: str = Field(..., description="Extracted or human-verified text transcript")
    extraction_method: str = Field(..., description="Method used to obtain transcript")
    category: str = Field(..., description="Validated topic category")
    concepts: list[str] = Field(..., description="Extracted technical concepts and tags")
    difficulty: DepthLevel = Field(..., description="Validated technical depth level")
    ...
    human_qc_record: dict[str, Any] | None = Field(default=None, description="Details of human review operation")
    extracted_signals: dict[str, Any] = Field(default_factory=dict, description="ScrollSense semantic signal extraction record")
    provenance: dict[str, Any] = Field(default_factory=dict, description="Audit trace of extractor and gate results")
    file_sha256: str = Field(..., description="SHA256 checksum of raw binary asset")
